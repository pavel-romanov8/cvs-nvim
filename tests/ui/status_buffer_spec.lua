...
return function()
  ...
  assert_eq(
    vim.fn.maparg("o", "n", false, true).desc,
    "Open current file in a matching split",
    "layout-matching split mapping"
  )
  ...
  service.open_current(bufnr, "layout")
  local matching_win = vim.api.nvim_get_current_win()
  local status_position = vim.api.nvim_win_get_position(winid)
  local matching_position = vim.api.nvim_win_get_position(matching_win)
  assert_eq(matching_position[2], status_position[2], "o follows a horizontal status layout")
  assert_true(matching_position[1] ~= status_position[1], "o creates a horizontal split for horizontal status")
  vim.api.nvim_win_close(matching_win, true)
  ...
  vim.cmd("wincmd H")
  service.open_current(bufnr, "layout")
  matching_win = vim.api.nvim_get_current_win()
  status_position = vim.api.nvim_win_get_position(winid)
  matching_position = vim.api.nvim_win_get_position(matching_win)
  assert_eq(matching_position[1], status_position[1], "o follows a vertical status layout")
  assert_true(matching_position[2] ~= status_position[2], "o creates a vertical split for vertical status")
  vim.api.nvim_win_close(matching_win, true)
  vim.api.nvim_set_current_win(winid)
  vim.cmd("wincmd J")
