...
return function(root)
  ...
  local ok, err = pcall(function()
    ...
    assert(text(full_buf):find("@@ -1 +1 @@", 1, true) and text(full_buf):find("+new", 1, true), "full view shows changed hunks")
    ...
    assert(text(bufnr):find("| @@ -1 +1 @@", 1, true) and text(bufnr):find("| +new", 1, true), "inline shows diff, not full file")
    ...
    assert(seen.DiffAdd and seen.DiffDelete and seen.DiffChange, "inline diff additions, removals, hunks highlighted")
    ...
    assert(not text(bufnr):find("| +new", 1, true), "inline diff removed")
